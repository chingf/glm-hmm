...
def main(coef_mode, trans_mode):
    ...
    if trans_mode == "std":
        ...
        np.random.rand(K, set_coef.shape[1])
        transitions = "inputdriven"
    elif trans_mode == "indHard":
        np.random.rand(K, set_coef.shape[1])*2
        transitions = "inputdriven"
    ...
    results_K = {}
    datainfo_K = {}
    datainfo_K['sim_X_train'] = sim_X_train
    datainfo_K['sim_y_train'] = sim_y_train
    datainfo_K['sim_X_test'] = sim_X_test
    datainfo_K['sim_y_test'] = sim_y_test
    for K in Ks[:1]: # TODO: remove
        results = _fit_hmm(
    ...
    dsizes = [200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100]
    results_dsize = {}
    datainfo_dsize = {}
    for dsize in dsizes[:1]: # TODO: remove
        results, datainfo = _fit_hmm_datasize(true_hmm, dsize, transitions)
        results_dsize[dsize] = results
        datainfo_dsize[dsize] = datainfo

    all_results = {}
    all_results['true_hmm'] = true_hmm
    all_results['results_K'] = results_K
    all_results['datainfo_K'] = datainfo_K
    all_results['results_dsize'] = results_dsize
    all_results['datainfo_dsize'] = datainfo_dsize
...
def _fit_hmm_datasize(true_hmm, train_size, transitions):
    test_size = 120
    X_train = np.random.choice(
        np.arange(-30,31,1), size=train_size).reshape((-1,1))
    X_test = np.random.choice(
    ...
    results = _fit_hmm(
        ...
        sim_X_test, sim_y_test, transitions)
    datainfo = {}
    datainfo['sim_X_train'] = sim_X_train
    datainfo['sim_y_train'] = sim_y_train
    datainfo['sim_X_test'] = sim_X_test
    datainfo['sim_y_test'] = sim_y_test
